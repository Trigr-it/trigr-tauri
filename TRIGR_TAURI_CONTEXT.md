...
## 10 — Featurebase Feedback Widget (replaces Google Form)

In-app feedback / bug-report flow now routes through Featurebase instead of a Google Form. Tester-facing experience is unchanged: same Settings button, same label ("Send Feedback"), same location. Only the click handler is different.

- **SDK loader in `index.html`.** Inline bootstrap script creates a queueing stub on `window.Featurebase` (calls made before the remote script lands get buffered and replayed) and injects `https://do.featurebase.app/js/sdk.js` async. This runs in every window (all five windows share `index.html`) — that's fine because the loader only installs the stub + a remote `<script>` tag, no widget UI mounts on its own.
- **Init in `src/App.jsx`** (`featurebaseInitedRef` + effect depending on `theme`). Three-arg signature: `Featurebase('initialize_feedback_widget', options, callback)`. Options: `{ organization: 'trigr', theme, placement: 'right', defaultBoard: 'feature-requests', locale: 'en' }`. **Do NOT pass `appId` here — that's a Messenger/SSO field and the validator rejects it for `initialize_feedback_widget`.** **Do NOT pass `email: ''` — empty string fails validation; either pass a real email or omit.** The callback (3rd arg) receives `(err, action)` — log errors, log `action.action === 'feedbackSubmitted'` to console as a debug hook. URL-param guard short-circuits if `overlay`/`fillin`/`radialmenu`/`clipboardoverlay` is present (defence-in-depth — App.jsx is already only mounted in the main window per `main.jsx` routing). Effect depends on `theme` so the latest value at first run is picked up even if config-loaded theme arrives after mount; the ref keeps it one-shot. Theme is not re-synced on later toggles by design.
- **Triggers** live in `src/components/SettingsPanel.jsx` (Send Feedback button in the Help & Support section) and `src/components/TitleBar.jsx` (the always-visible Trigr-branded "Feedback" pill in the titlebar right cluster, mirroring `.tb-templates-btn`). Both call `openFeedback()` from `src/utils/feedback.js`, which posts `{ target: 'FeaturebaseWidget', data: { action: 'openFeedbackWidget', setBoard: 'feature-requests' } }` via `window.postMessage`. If `window.Featurebase` is undefined (offline at boot, CDN blocked, CSP misconfig) it falls back to `mailto:admin@usetrigr.com` via `openExternal` so the trigger is never a dead click. **Featurebase's auto-rendered edge tab is suppressed by omitting `placement` from the init options** — passing any `placement` value (`right`/`left`/etc.) makes the SDK draw its own tab, which fights with Trigr's right-side UI cluster.
- **CSP additions in `src-tauri/tauri.conf.json`.** `script-src` and `font-src` gained `https://do.featurebase.app`; `style-src` gained `https://do.featurebase.app`; `img-src` and `connect-src` and `frame-src` gained `https://*.featurebase.app`. Without these the SDK script and widget iframe are blocked.
- **Debug hook.** The init call's third-arg callback is the only listener wiring needed — `(err, action) => { ... }` runs for both validation errors and `feedbackSubmitted` actions. There is no separate `on_feedback_submitted` action on the SDK (it errors with "Action ... is not supported"). No toast on success; console log only.
- **No npm dependency.** Featurebase SDK is loaded via script tag only; `package.json` is unchanged. No `featurebase-js` import anywhere in the tree.
