...
> Last updated: 2026-04-08 (post v0.1.30)
...
**Gold accent styling:** Both CSS files match Quick Search overlay design language. Search bars use `var(--border-accent)` (gold tint) border + `var(--bg-elevated)` background + `caret-color: var(--accent)`. No drop shadows on panels/cards/context menus. Card hover uses `border-color` transition instead of `transform`+`box-shadow`. Tag pills, timeline headers, button hovers all use `var(--accent)` gold. Overlay panel border: `var(--border-accent)` instead of `var(--border-bright)`. Overlay row hover: `var(--accent-glow)` instead of `var(--bg-hover)`.
